...
class AppUtils {
    ...
    {
        UIGraphicsBeginImageContextWithOptions(CGSize(width: 320, height: 150), false, 0);
        let context = UIGraphicsGetCurrentContext();
        let colorspace = CGColorSpaceCreateDeviceRGB();
        
        let colors = [#colorLiteral(red: 0, green: 0.5848664726, blue: 0.7607843137, alpha: 1).cgColor, #colorLiteral(red: 0, green: 0.8431372549, blue: 0.6, alpha: 1).cgColor] as CFArray;
        
        let gradient = CGGradient(colorsSpace: colorspace, colors: colors, locations: nil);
        context!.drawLinearGradient(gradient!, start: CGPoint(x:0, y:0), end: CGPoint(x:CGSize(width: 320, height: 150).width, y:0), options: CGGradientDrawingOptions(rawValue: UInt32(0)));
