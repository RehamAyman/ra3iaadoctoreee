...
class ChatPrisnter {
    ...
    func SoundCellConfigeration(cell:SoundTableViewCell,for Index:Int){
        ...
        cell.setContent(msg: self.MessagesArray[Index].content)
        print("🎁🎁\(self.MessagesArray[Index].content)")
    }
    ...
    func didSelectLocation(index:Int){
       var location =  self.MessagesArray[index].content.toJSON() as! [String : Any]
        self.view?.goToLoctionDetils(lat: location["lat"] as! Double, lng: location["lang"] as! Double)
    }
